            ...
            try:
                async with httpx.AsyncClient() as client:
                    response = await client.post(
                        self.config.discord_webhook_url, json=payload, timeout=10.0
                    )
...
async def notify_workflow_started(
    service: NotificationService, role_name: str, execution_id: int
) -> bool:
    ...
    return await service.send(
        Notification(
            ...
            data={"role_name": role_name, "execution_id": execution_id},
        )
    )


async def notify_workflow_completed(
    service: NotificationService, role_name: str, execution_id: int, summary: dict
) -> bool:
...
async def notify_wave_completed(
    service: NotificationService, wave: int, wave_name: str, success_count: int, failure_count: int
) -> bool:
...
async def notify_human_needed(
    service: NotificationService, role_name: str, execution_id: int, reason: str, node_name: str
) -> bool:
